import csv
# gearRatio = 50.0 # geared 50.0x slower
inertia = 0.1 # 10.0 kgm^2 mechanism inertia without gearbox
startVelocity = 0.0 # mechanism starts at 0.0 RPM
distanceToTravel = 35.0 # mechanism must travel 5.0 rotations (without gearbox factored in)
...
while (currentPosition < distanceToTravel) :
    acceleration = getTorque(currentVelocity * 60) / inertia
...
print ("total time for a mechanism with inertia of " + str(inertia) + " kgm^2 to travel " + str(distanceToTravel) + " rotations: " + str(round(currentTime,2)) + " seconds")